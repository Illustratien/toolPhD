...
#' @examples
#' library(ggplot2)
#' p <- ggplot(mtcars,aes(x=disp,y=hp)) + geom_point()
...
theme_phd_main<- function(ax.txt.siz=NULL,ax.tit.siz=NULL,lgd.txt.siz=NULL,lgd.tit.siz=NULL,strp.txt.siz=NULL,
  ...
  if (is.null(strp.txt.siz)){strp.txt.siz <- 2}
  if(is.null(t)){t<-10}
  if(is.null(r)){r<-10}
  if(is.null(b)){b<-10}
  if(is.null(l)){l<-10}
  ...
  ggplot2::theme_classic()+
    ggplot2::theme(
      ...
      axis.title.y = element_text(margin = margin(r=3),vjust=-2),# spacing of y and tick
      axis.title.x = element_text(margin = margin(b=3),vjust=-2), # spacing of x and axis
      # axis tick
